...
function mlReferences_delete($directory)
{
...
function mlReferences_filters_author($item)
{
    return $item['role'] === 'Author';
}

function mlReferences_filters_authors($item)
{
    if ($item === 'Ed') {
        return false;
    }
...
function mlReferences_filters_editor($item)
{
    return $item['role'] === 'Editor';
}

function mlReferences_log($contents)
{
...
function mlReferences_uasort($one, $two)
{
...
function mlReferences_usort($one, $two)
{
    return strlen($two) - strlen($one);
}

function mlReferences_zip_get_articles($file)
{
    $articles = array();
    ...
    if ($contents !== false) {
        $articles = mlReferences_zip_get_items($contents);
        return $articles;
    }
...
function mlReferences_zip_get_authors($file)
{
    $authors = array();
    ...
    if ($contents !== false) {
        $authors = mlReferences_zip_get_items($contents);
        return $authors;
    }
...
function mlReferences_zip_get_articles_authors($file)
{
    ...
    if ($contents !== false) {
        $articles_authors = mlReferences_zip_get_items($contents);
        return $articles_authors;
    }
...
function mlReferences_zip_get_items($contents)
{
    $items = array();
...
function mlReferences_get_access_date($access_date)
{
...
function mlReferences_get_author($name)
{
    $explode = array();
...
function mlReferences_get_authors($item)
{
    $authors = array();
...
function mlReferences_get_citations_first($authors, $year)
{
    $count = count($authors);
    if ($count === 0) {
        return '';
...
function mlReferences_get_citations_parenthetical_first($authors, $year)
{
    $count = count($authors);
    if ($count === 0) {
        return '';
...
function mlReferences_get_citations_parenthetical_subsequent($authors, $year)
{
    $count = count($authors);
    if ($count === 0) {
        return '';
...
function mlReferences_get_citations_subsequent($authors, $year)
{
    $count = count($authors);
    if ($count === 0) {
        return '';
...
function mlReferences_get_csv_dialect()
{
...
function mlReferences_get_directory($items)
{
...
function mlReferences_get_endnote($item, &$txt)
{
    ...
    mlReferences_log('Article');
    mlReferences_log(sprintf('    Title          : %s', $item['title_1']));
    mlReferences_log(sprintf('    Year           : %s', $item['year']));
    mlReferences_log(sprintf('    Publisher      : %s', $item['publisher']));
    mlReferences_log(sprintf('    Place Published: %s', $item['place_published']));
    mlReferences_log('    Authors:');
    foreach ($item['authors'] as $author) {
    ...
    foreach ($txt as $key => $value) {
        ...
        mlReferences_log(sprintf('Checking if "%s" is a match...', $value));
        ...
        if ($item['title_1'] === $title_1) {
            ...
            mlReferences_log('    Step 1: Title is not a match');
        }
        ...
        if ($statuses['authors'] === false) {
            ...
            if ($count_2 === $count_1 && $count_1 > 0 && $count_2 > 0) {
                ...
                mlReferences_log('    Step 2.1: Authors are a match');
            } else {
                mlReferences_log('    Step 2.1: Authors are not a match');
            }
        }
        if ($statuses['authors'] === false) {
            $authors_2 = $value[0];
        ...
        if ($statuses['title'] === true and $statuses['authors'] === true) {
            ...
            mlReferences_log('    Failure: We do not have a match!');
            mlReferences_log('    Continuing onto the next line...');
        }
    }
    mlReferences_log('Failure: None of the lines in the TXT file were a match!');
    mlReferences_log(str_repeat('-', 80));
    return -1;
}

function mlReferences_get_file($items)
{
...
function mlReferences_get_items($xml, $txt)
{
    $items = array();
    ...
    foreach (@simplexml_load_string($xml)->xpath('//xml/records/record') as $key => $value) {
        try {
            ...
            usort($item['isbn'], 'mlReferences_usort');
            ...
            $item['access_date'] = mlReferences_get_access_date($item['access_date']);
            ...
            $item['references_editors'] = mlReferences_get_references_editors($item);

            $item['references_all'] = mlReferences_get_references_all($item);

            $item['endnote'] = '';
            $endnote = mlReferences_get_endnote($item, $txt);
            if ($endnote !== -1) {
            ...
            return array(sprintf('mlReferences_get_items() - %s', $exception->getMessage()), array(), array());
        }
    }
...
function mlReferences_get_initials($name)
{
    $initials = array();
    $name = explode(' ', $name);
    if (!empty($name)) {
...
function mlReferences_get_prefix()
{
    return sprintf('%smlReferences_', $GLOBALS['wpdb']->prefix);
}

function mlReferences_get_references_all($item)
{
    return sprintf(
        '%s. %s: %s. %s: %s',
...
function mlReferences_get_references_authors($item)
{
    $item['authors'] = array_filter($item['authors'], 'mlReferences_filters_author');
...
function mlReferences_get_references_editors($item)
{
    $item['authors'] = array_filter($item['authors'], 'mlReferences_filters_editor');
    $count = count($item['authors']);
    if ($count === 0) {
        return '';
    }
...
function mlReferences_get_shortcodes($contents)
{
    $items = array();
    preg_match_all(
        ...
        foreach ($matches as $match) {
            ...
            $article = $GLOBALS['wpdb']->get_row(
                $GLOBALS['wpdb']->prepare(
                    sprintf("SELECT * FROM `%sarticles` WHERE `id` = %%d", mlReferences_get_prefix()),
                    intval($match[1])
                ),
                ARRAY_A
            );
...
function mlReferences_get_url($first_name, $last_name)
{
...
function mlReferences_get_value($value_1)
{
...
EOD;
    $GLOBALS['wpdb']->query(sprintf($query, mlReferences_get_prefix()));
...
function mlReferences_register_deactivation_hook()
{
    ...
    $GLOBALS['wpdb']->query(sprintf('DROP TABLE IF EXISTS `%sarticles_authors`', mlReferences_get_prefix()));
    $GLOBALS['wpdb']->query(sprintf('DROP TABLE IF EXISTS `%sauthors`', mlReferences_get_prefix()));
    $GLOBALS['wpdb']->query(sprintf('DROP TABLE IF EXISTS `%sarticles`', mlReferences_get_prefix()));
    $GLOBALS['wpdb']->query(sprintf('DROP TABLE IF EXISTS `%sdocuments`', mlReferences_get_prefix()));
}
...
function mlReferences_add_meta_boxes()
{
    add_meta_box(
        ...
        'mlReferences_add_meta_boxes_1',
        'page'
    );
    add_meta_box(
        'mlReferences_2',
        'References Management - Translations',
        'mlReferences_add_meta_boxes_2',
        'page'
    );
    add_meta_box(
        'mlReferences_3',
        'References Management - Semantic Annotations',
        'mlReferences_add_meta_boxes_3',
        'page'
    );
    add_meta_box(
        'mlReferences_4',
        'References Management - References',
        'mlReferences_add_meta_boxes_4',
        'page'
    );
}
    ...
    <div class="mlReferences_1">
        <table class="mlReferences_widget">
            <tr class="even">
                ...
                <td>
                    <select
                        id="mlReferences_1_multipage_report"
                        name="mlReferences_1_multipage_report"
                        >
...
EOD;
    $articles = $GLOBALS['wpdb']->get_results(sprintf($query, mlReferences_get_prefix()), ARRAY_A);
    ?>
...
if (defined('WP_CLI') && WP_CLI) {
    class mlReferences_cli
    {
        ...
        public function upload($args)
        {
            ...
            list($errors, $items, $txt) = mlReferences_get_items($xml, $txt);
            foreach ($items as $item) {
                if ($item['endnote'] === '') {
                    foreach ($item['authors'] as $key => $value) {
        ...
        public function uploadZip($args)
        {
            ...
            print_r($articles);
            $authors = mlReferences_zip_get_authors($file);
            $authors = array_slice($authors, 0, 5);
            print_r($authors);
            $articles_authors = mlReferences_zip_get_articles_authors($file);
